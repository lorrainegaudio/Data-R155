...
* For Tasks 4–7, counts and countries will vary by the student’s year/region choices; grade correctness of logic and syntax ([], logical operators, %in%, negative column indices, which()), not the exact numbers.

* Explanations/memos should be short, accurate, and use correct vocabulary (logical vector, rows/cols slots, membership, AND/OR).

**Campus NGO Health Pilot** 🌍💉
...
✍️ LEARNING SKILL MEMO (1–2 sentences): Compare functions `colnames()` and `names()` output.
...
✍️ MEMO (1–2 sentences): “I chose target_year = 2010 because it’s recent enough to reflect current patterns and is present in the dataset.”
...
is_year   <- gapminder$year == target_year            # Fill in: year column, target year object
is_region <- gapminder$region %in% c(region_1, region_2)  # Fill in: region column, region 1, region 2

# TODO 3-B: combine
keep <- is_year & is_region            # combine with AND
...
```{r}
# Count countries with life expectancy ≥ 70
sum(focus_pool$life_expectancy >= 70)

# (Optional extra from prompt: also ≥ 60)
