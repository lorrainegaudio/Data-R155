...
🎯 Identify the exact region names; then pick (your choice) your two target regions.
...
*Use this output to select two regions. Copy exact names.*
...
```{r , eval=FALSE}
# ⚡ Type this in your R Notebook, Fill in the blanks
# TODO 2-B: store two regions EXACTLY as shown above
region_1 <- "___"
region_2 <- "___"
```

🗣 I selected ___ and ___ as my target regions because _____.

## Task 3 🌱

🔍 Focus the Pool

🎯 Create `focus_pool` that keeps rows from the `target_year` AND restricts to two regions you want to prioritize. 
...
is_region <- _________$region ___ c(region_1, region_2)  # test for membership in the set
```
...
Use negative indexing to drop the columns 6, 7, 8, and 9 from `focus_pool`. 

```{r , eval=FALSE}
# ⚡ Type this in your R Notebook, Fill in the blanks
brief_sheet <- focus_pool[ , -c(___, ___, ... ___) ]    # use c() or :
# ✅ Verify
names(______)
```
...
# ⚡ Type this in your R Notebook, Fill in the blanks
# The value `a` in Task 6 will be based on this range and count.
sum(brief_sheet$life_expectancy <= ___)  
```
...
# ⚡ Type this in your R Notebook, Fill in the blanks
# The value `b` in Task 6 will be based on this range and count.
sum(brief_sheet$infant_mortality >= ___)
```
...
🎯 Now that you have inspected the ranges and counts for each indicator, you can set thresholds for each one to identify countries that need the health pilot the most. Use what you learned from Task 5 to determine appropriate cutoff values for `life_expectancy` (a), `infant_mortality` (b), and `fertility` (c).
...
🎯  Show that filtering the same two regions with `|` is equivalent to using `%in%`.

**Step 1** Use `|` to filter for the same two regions you used in Task 3.
...
A_or_B <- gapminder[
  gapminder$year == target_year &
    (gapminder$region == region_1 ___ gapminder$region == region_2), # Which operator goes here?
]
```
 **Step 2** Use `%in%` to filter for the same two regions.
...
in_set <- gapminder[
  gapminder$year == target_year &
    gapminder$region ___ c(region_1, region_2), # Which operator goes here?
]
```
