...
Goal: Keep original type; add full_type with a descriptive phrase.

Reference code (example mapping; any clear, consistent mapping earns credit)
```{r}
data(stars)  # dslabs

stars_recode <- stars %>%
  dplyr::mutate(
    full_type = dplyr::case_when(
      type == "O" ~ "O-type (blue)",
      type == "B" ~ "B-type (blue-white)",
      type == "A" ~ "A-type (white)",
      type == "F" ~ "F-type (yellow-white)",
      type == "G" ~ "G-type (yellow)",
      type == "K" ~ "K-type (orange)",
      type == "M" ~ "M-type (red)",
      TRUE        ~ "Other"
    )
  )
...
```{r}
exists("stars_recode")
all(c("type","full_type") %in% names(stars_recode))
head(dplyr::select(stars_recode, type, full_type))
```

Acceptable variants: any readable phrase per code; a default TRUE ~ "Other" is fine.
Deduct if: overwrote type; full_type missing or meaningless (e.g., just copies type with no description).
