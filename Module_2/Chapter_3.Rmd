...
**Vectorization** is when R applies an operation to each element of a vector without needing an explicit loop. In the example above, R adds the corresponding elements of vectors `x` and `y` together element-by-element.
...
Next, you will learn about data types and how to create different kinds of vectors in R. Write memo as you practice R programming. 

# Vector Metadata

class, names, and indexing 
...
nerdy_palette <- c("Octarine" = "#7F00FF", "TARDIS Blue" = "#003B6F", "Rebeccapurple" = "#663399", "Hooloovoo" = "#4F97A3", "Cosmic Latte" = "#FFF8E7", "Vantablack" = "#000000", "Matrix Green" = "#03A062", "Source Engine Magenta" = "#FF00FF", "Command Red" = "#CC0000", "Impossible Green" = "#4D9E68")
```
...
Fill in the blanks to complete the code. Before you begin, make sure you have created a new R Notebook in the Source pane and saved it as `chapter2_practice.Rmd` in your course folder. Remember to document your tenacity on a memo if you encounter any errors while completing the tasks. Additionally, sweeping your environment before running the code. When you finish, sweep your environment then "Run All" to test that your entire notebook  to ensure it runs without errors from top to bottom. Save and submit your R notebook (`.Rmd`) and environment (`.RData`) to Canvas, as directed.
...
```{r , eval=FALSE}
# ⚡ Type this in your R Notebook, Fill in the blanks
nerdy_palette <- c("Octarine" = "#7F00FF", "TARD
IS Blue" = "#003B6F", "Rebeccapurple" = "#663399", "Hooloovoo" = "#4F97A3", "Cosmic Latte" = "#FFF8E7", "Vantablack" = "#000000", "Matrix Green" = "#03A062", "Source Engine Magenta" = "#FF00FF", "Command Red" = "#CC0000", "Impossible Green" = "#4D9E68")
```
```{r , eval=FALSE}
# ⚡ Type this in your R Notebook, Fill in the blanks
...
```{r , eval=FALSE}
# ✅ Check the output
print(nerdy_palette[c("Matrix Green", "Command Red")])
```
...
🗣 The output shows the values in the `growth` vector that are greater than zero in the `positive_growth` vector, and the last two elements of the `sales_q2` vector in the `last_two` vector.
