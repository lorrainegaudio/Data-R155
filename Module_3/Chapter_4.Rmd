...
- 🗜️ Summarize vectors with built‑in, *vectorised* functions.
...
🎯 Practice creating headings in your R Notebook. 
...
When you select to knit, the Console pane shows progress messages in the **Render tab** as R runs your notebook from top to bottom. A successful knit has three visible signs:
...
The formating of your prompts can greatly influence the quality of responses from LLMs. There are several different ways to structure prompts. The more structured the prompt, the better the response.

Plain Text → Markdown → Delimiters → Encapsulation → PascalCase → XML → YAML → JSON

In this section, we will look at structuring prompts using plain text and **Markdown**. Markdown is the easier and most common way to structure prompts.

## Missing Backtick

Imaging you don't know that there is a missing backtick at the end of a code chunk. You try to knit to HTML, but it fails. You ask an LLM to help you find the error.
