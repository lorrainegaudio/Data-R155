...
| `seq()`        | Sequences                  | `seq(1, 10, by = 2)`         |
| `rep()`        | Repeat values              | `rep(1:3, times = 2)`        |
| `append()`     | Insert/append into vector  | `append(1:3, 99, after = 2)` |
| `:`(colon)     | Integer sequence shorthand | `1:10`                       |
| `unique()`     | Keep unique elements       | `unique(c(1,1,2))`           |
| `duplicated()` | Flags repeated elements    | `duplicated(c(1,1,2))`       |
| `rev()`        | Reverse order              | `rev(1:5)`                   |


3. Example *mean**

⚖️ Type the following code in your R script and run each line.
...
| `quantile()`                | Quantiles        | `quantile(x, probs = .25)` |


📍 Identify: Try `median(values, na.rm = TRUE)`. Why doesn’t `median() `need trim?

🚀 Explore and Play: What happens if you try  sum vs. sum(1:5)? Try out the descriptive statistics and summaries functions on objects.


```{r}
# Create a memo note in your R script if you did 📍 Identify, and 🚀 Explore and Play. 
...
**Sorting, Ordering, Rounding & Formatting**

| Function  | What it does         | Example                            |
| --------- | -------------------- | ---------------------------------- |
| `sort()`  | Sort values          | `sort(x, decreasing = TRUE)`       |
| `order()` | Order indices        | `x[order(x)]`                      |
| `rank()`  | Ranks (ties handled) | `rank(x, ties.method = "average")` |
| `round()` | Round to digits      | `round(runif(5), 2)`               |
| `format()`| Pretty formatting    | `format(Sys.Date(), "%B %d, %Y")`  |
| `rev()`   | Reverse order        | `rev(1:5)`                         |
| `as.numeric()`| Coerce vector    | `as.numeric("1", "2", "3")`        |
| `as.character()`| Coerce vector  | `as.character(1:3)`                |
| `length()`| Count elements       | `length(x)`                        |
| `head()`  | First part           | `head(x, 3)`                       |
|`tail()`   | Last part            | `tail(x, 3)`                       |

🧠 Challenge: Draw 5 numbers without replacement and sort them ascending.


✍️ Practice: From the functions we have learned about, pratice nesting. 

```{r}
# Create a memo note in your R script if you did 🧠 Challenge and ✍️ Practice.
