...
movielens1 <- movielens %>%
  filter(!is.na(rating) & !is.na(year), year>= 1990) %>%
  select(rating, year)
```
```{r}
# ✅ Verify: not missing ratings and years
is.na(movielens1$rating) %>% any()
is.na(movielens1$year) %>% any()

# ✅ Verify: years are 1990 or greater
all(movielens1$year >= 1990)
```
...
```{r}
# ✅ Verify:
is.factor(movielens3$rating_label)
identical(levels(movielens3$rating_label), c("Bad", "Good"))
```
...
Movie_Facts <- movielens %>%
  ...
  arrange(desc(mean_rating))  # Chose to sort by mean_rating descending
```
```{r}
# ✅ Verify:
identical(names(Movie_Facts), c("decade","rating_label","mean_rating","n"))
!dplyr::is_grouped_df(Movie_Facts)
```
...
📝 Write a short paragraph reflecting on what `n` is counting. Is it individual movies or individual ratings? Explain. 

✍️ EXPLANATION: "___"

# Save and Upload

1. You will be submitting **both** the "your choice" Document and the workspace file. The workspace file saves all the objects in your environment that you created in this lesson. You can save the workspace by running the following command in a code chunk of the Quarto Document document:
