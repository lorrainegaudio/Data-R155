...
In the code blocks below, underscores (_) represent blanks. Do not type underscores. Replace them with the correct code (object names, function names, numbers, commas/quotes, or parentheses).
...
# 🎯 Build a numeric vector named `exponents` that contains the integers from 0.5 to 10 using the `seq()` function.
...
```{r eval=FALSE}
# 🎯 Create a data frame using `data.frame()` named `nerdy_df` that contains three columns: "Origin" (`origin`), "Color" (`color_names`), and "Hex" (`nerdy_palette`). 
...
Read the following vignettes. Answer the question. Include one reason and one condition/limit. Paste your answer as 2–3 comment in Module Canvas as directed.
