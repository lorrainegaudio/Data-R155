...
Replace each ____ placeholder (and any TODO comments) with working code or a short written answer. Run each section; be sure the requested objects appear in the Environment. Write 🎯/❓ Goal → Prediction → ⚡ Code → ✅ Checks → 🗣/🪞 Reflection to show how you composed your answer. 

When finished, save **BOTH** this script and your .RData workspace and upload.
___

🔥 How far could a new fire run in 8 hours?

Today, you’ll simulate 8-hour spread distance (km) for new ignitions in Idaho. We’re using Idaho’s Wildfire Hazard Potential (WHP) only to define categories and their probabilities (area shares). 

Researchers use Idaho’s WHP to describe where intense wildfire is more likely. Below is a WHP map for your viewing pleasure. I've already converted the map to hazard categories and probabilities (area shares) for Idaho into a table. 

```{r echo=FALSE, message=FALSE, warning=FALSE, results = 'hide', fig.cap='A raster of wildfire hazard potential for Idaho. This depicts the relative potential for high-intensity wildfire behavior across Idaho based on factors like vegetation, topography, and historical fire data. Areas with higher WHP values indicate a greater likelihood of experiencing severe wildfire events, which is crucial information for land management, firefighting strategies, and community preparedness in wildfire-prone regions.'}
...
We’re going to model “how far a new fire could run in 8 hours” for different hazard levels in Idaho. You don’t need a map. I’ve already turned the statewide wildfire-hazard map into seven categories and the chance a random ignition would fall in each one. For the first practice step, you’ll pretend distances in the High category follow a simple bell-shaped pattern (a Normal curve) with a typical distance of 7 km and a typical spread of 2 km. That’s just to warm up with rnorm(). After that, you’ll switch to a Lognormal shape for every category. “Lognormal” just means we generate a distance in a way that keeps it positive and allows a longer right tail, which matches the idea that most runs are moderate but a few runs can be much longer.

Use Table 1,to drive your simulations. The “Probability” column is only needed later when we mix categories; you won’t need it for Task 1. The “Normal mean/SD” columns give you the practice values for rnorm() (use the High row for Task 1). The “Mean log / SD log” columns give you the realistic Lognormal settings for each category.

```{r echo=FALSE, message=FALSE, warning=FALSE, tab.cap='Table 1. Idaho hazard categories (area-share probability) and distance models for an 8-hour run.'}
library(dplyr)
library(knitr)
library(kableExtra)

param_tbl <- tibble::tibble(
  Category = c("Very Low","Low","Moderate","High","Very High","Extreme","Ultra (Top 1%)"),
  Probability = c(0.25, 0.25, 0.25, 0.15, 0.05, 0.04, 0.01),
  `Normal mean (km)` = c(2.0, 3.0, 4.5, 7.0, 12.0, 18.0, 28.0),
  `Normal SD (km)`   = c(1.0, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0),
  `Mean log` = c(0.582, 1.046, 1.451, 1.907, 2.455, 2.866, 3.310),
  `SD log`   = c(0.472, 0.325, 0.325, 0.280, 0.246, 0.220, 0.212)
) %>%
  mutate(
    Probability        = sprintf("%.0f%%", Probability * 100),
    `Normal mean (km)` = sprintf("%.1f", `Normal mean (km)`),
    `Normal SD (km)`   = sprintf("%.1f", `Normal SD (km)`),
    `Mean log`         = sprintf("%.3f", `Mean log`),
    `SD log`           = sprintf("%.3f", `SD log`)
  )
...
Normal baseline (`rnorm()`)

Make 50 numbers that represent possible 8-hour distances for the High category using a Normal curve with mean 7 km and SD 2 km (from the table). If any values come out negative, change them to 0 because distance can’t be negative. Save your results as `High_Normal`. Then print a short summary (minimum, quartiles, and maximum), report the mean and SD you actually got, and draw a histogram. In one or two sentences, explain why roughly half of your values should sit below 7 km.
...
```{r}
# ⚡ Test
High_Normal <- rnorm(n = 50, mean = 7, sd = 2
)
```

✅ Checks:  provide a 5 number summary of High_Normal and draw a histogram.
...
🗣 Report length, mean, SD, and draw a histogram.


## Task 2

The Normal model was your practice baseline. The Lognormal model is more realistic because the actual data is skewed.

🎯 Goal: Create x = 50 simulated 8-hour distances for High using a Lognormal model.
...
- `meanlog` is the mean of the logarithm of the distribution.

- `sdlog` is the standard deviation of the logarithm of the distribution.

❓ Prediction: Compared to Task 1, which tail will increase? 

⚡ Code 

- Use meanlog = 1.907 and sdlog = 0.280 (Table 1) to generate 50 Lognormal draws.

Save as your High_Lognorm vector.

```{r}
# ⚡ Test
High_Lognorm <- rlnorm(n = 50, meanlog = 1.907
, sdlog = 0.280)
```

✅ Checks:

Report median, mean, 90th percentile; make a histogram and point out the right tail.
...
🎯 Goal: Create one combined sample of N = 200 distances for “a random ignition in Idaho.”

❓ Prediction: Will the combined histogram look more like Moderate/High or Very High/Extreme? Why?

⚡ Code (describe):

First, sample categories using the probabilities in Table 1 (25%, 25%, 25%, 15%, 5%, 4%, 1%).

For each sampled category, draw one Lognormal distance using that category’s parameters (Table 2).

Collect all distances in one vector and plot a histogram.
