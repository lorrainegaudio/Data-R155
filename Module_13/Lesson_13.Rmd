...
In lesson 12, we did simple data visualizations using R’s built‑in function `hist()`. Today, we use the package `ggplot2` to turn data frames into clear, publication‑quality plots. Specificaly, we'll look at the grammar of graphics.
...
🧐 NOTICE: Which argument supplies mapping information?

🧩 Explanation: In ggplot2, mapping refers to connecting your data variables to visual properties (aesthetics) of the plot. It's how you tell ggplot which column in your data should control which aspect of the visualization.
...
The result is empty — we still need aesthetic mappings and a geom.
...
🎯 The GOAL: Map divorce rate to x and margarine consumption to y.
...
🧩 Explanation: `geom_point()` understands these aesthetics:
...
🧐 NOTICE: that the first country, "USA" will have a mean height of 67 inches
...
🚀 Explore and Play: Change `binwidth = 0.25` inside `geom_histogram()`. How does the distribution look?

🛠 Break Things! Test out your hand at adding aesthetics `geom_histogram()`  understands.

# Group Comparison

🎯 The GOAL: Overlay two histograms by mapping fill to Country. 
...
👀 Check‑in: Which country shows greater height variability?
...
✍️ Practice & 🚀 Explore and Play: 
  ...
  3. Use `iris` data to create side‑by‑side boxplots of `Sepal.Width` by `Species` and change the theme.

Add your code below and include at least one 👀 check‑in to predict an outcome.
...
When you’re done, your workspace should contain FOUR new objects: `Pretty_Flowers`, `Petal_Spread`, `CO2_Graph`, `Tall_Density`
...
Build **`Pretty_Flowers`**: `iris` data ➜ `Sepal.Length` vs `Petal.Length,` color points by `Species`, `size = 3`, add labels for both axes.
...
Create **`Petal_Spread`**: `iris` ➜ `Petal.Width` by `Species` as green boxplots (no legend).  Use `fill = Species`, then `show.legend = FALSE`.
