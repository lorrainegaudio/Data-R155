...
Create a simple outline for Chapter 12 using headings. Use the following structure:
...
Now that you have created (*and saved*) your quarto file, follow along this guided practice and taking notes.
...
🗣 `x` is a vector with the length of `5` and values `3`, `2`, `3`, `6`, and `5`.
...
`mutate()` creates new columns that are functions of existing columns. It can also modify an existing column if you reuse the same column name. In this course, the main idea is simple: `mutate()` lets you add a new variable to your data without changing the number of rows. 
...
`mutate()` + `case_when()` is a powerful way to create new variables based on complex rules. You can have as many conditions as you need, and you can use any existing columns in your data frame to build those rules
...
🗣 In `cars_overwrite`, the `am` column has been *changed* from `0`/`1` to `"Automatic"`/`"Manual"`. 
...
When the left side used a new name, R added a new column. When the left side reused an existing name, R overwrote that column.
...
Using `quakes`, recode `mag` into THREE ordered levels of _your choice_ (e.g., "Low", "Mid", "High"). Use `mutate()` and `case_when()` chain and pipe (`%>%`). Store as **`quake_recode`**.
...
With 🌟`stars`, classify each `star` by temperature range in a new column called `temp_group`. Keep the original `temp` column. Store the result as `stars_recode`. Temperatures less than 4,000 are "Cool", between 4,000 and 7,500 are "Warm" and all stars more than 7,500 are "Hot".
